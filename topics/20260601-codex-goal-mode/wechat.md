...
eyebrow: 一句话判断
title: Goal 更像验收单
body: 它的价值不在于让 agent 无限工作，而在于让长期任务的完成条件、验证方式和停止时机一直留在当前线程里。
:::

我查了一圈文档，也在本机确认了一下：`codex --version` 输出 `codex-cli 0.133.0`，`codex features list` 里 `goals` 是 stable。OpenAI 在 2026-05-21 的 Codex changelog 里写到，Goal mode 不再是 experimental，Codex app、IDE extension 和 CLI 都能用。官方的描述很克制：Goal 是 thread 上的 persistent objective，用来让 Codex 朝一个具体目标持续推进，可以暂停、恢复、清除。
...
OpenAI 的 Follow Goals 文档给了一个很实用的判断：Goal 适合 long-running work，尤其是 code migration、大重构、deployment retry loops、实验、游戏、side project 这种需要跨 turn 的任务；不适合一串无关小事。
...
:::cards[好 Goal 的四个要素]
目标 | 要达成什么 | 例如改首页、迁移支付模块、修复某条 CI | accent
边界 | 不要碰什么 | 限定目录、行为和产品边界 | default
验证 | 怎么证明进展 | 跑测试、看 diff、截图、写迁移清单 | default
停止 | 何时停 | 明确完成条件、失败预算和阻塞上报方式 | default
:::
...
目标最多 4000 个字符。长计划不要硬塞进去，放到文件里，让 Goal 指向文件。Codex app 的 commands 文档也写到，Goal active 时会在 composer 上方显示进度，你可以暂停、恢复、编辑或清除。
...
Ralph Loop 是 Anthropic verified 的 Claude Code 插件。它的公开实现很直白：根据 README，Ralph 用 Stop hook 拦截 Claude 试图结束 session 的动作，然后把同一个 prompt 再喂回去，让 Claude 继续干。你可以设置 `--max-iterations`，也可以设置 `--completion-promise`，比如“当你输出某句承诺时才算完成”。
...
这里还有一个容易忽略的事实：Claude Code 现在也有官方 `/goal`。Anthropic 的 goal 文档写得比 OpenAI 更暴露机制：Claude 每个 turn 后会用一个 small fast model 检查 completion condition 是否满足；文档还说 `/goal` 是一个 session-scoped prompt-based Stop hook wrapper。Claude 的 `/loop` 文档则把它定位成 session 内按间隔重复运行 prompt 或 slash command 的能力，更适合轮询状态或提醒。

:::cards[四种多轮机制]
Codex Goal | 线程目标 | 当前 thread 持续携带 persistent objective，适合有明确验收条件的长任务 | accent
Claude /goal | 完成条件 | turn 结束后由独立 evaluator 检查 completion condition，适合持续推进一个可验证条件 | default
Claude /loop | 定时任务 | 计时器到点后重新运行 prompt 或 slash command，适合轮询 CI、部署、PR 状态 | default
Ralph Loop | 退出拦截 | Stop hook 拦截退出后重放同一个 prompt，适合反复试错、跑测试、修失败 | default
:::
...
:::summary
eyebrow: 可以带走的判断
highlight: 开 Goal 之前，先写清楚完成条件。
body: 能写出验收单，Goal 才会变成长期任务的支点；写不出来，它只是把模糊愿望保存得更久。
:::

## 参考资料

1. OpenAI Codex changelog：https://developers.openai.com/codex/changelog
2. OpenAI Codex Follow Goals：https://developers.openai.com/codex/use-cases/follow-goals
3. OpenAI Codex CLI slash commands：https://developers.openai.com/codex/cli/slash-commands
4. OpenAI Codex app commands：https://developers.openai.com/codex/app/commands
5. Ralph Loop plugin：https://claude.com/plugins/ralph-loop
6. Ralph Loop README：https://github.com/anthropics/claude-code/blob/main/plugins/ralph-wiggum/README.md
7. Claude Code goal：https://code.claude.com/docs/en/goal
