...
Google DeepMind 在 [Gemini 2.5 技术报告](https://storage.googleapis.com/deepmind-media/gemini/gemini_v2_5_report.pdf) 的 Gemini Plays Pokémon 讨论中使用了 `context poisoning` 这个说法。报告 Appendix 8.2 的 Additional Challenges 里提到，目标、summary 等 context 片段可能被游戏状态的错误信息污染，模型会执着于不可能或无关的目标，而且这种污染可能需要很长时间才能解开。
...
论文层面也有旁证。[Contextual Drag](https://arxiv.org/abs/2602.04288) 研究了错误 context 对 LLM 推理的拖拽效应，论文摘要里提到，错误尝试进入 context 后会诱发结构相似的后续错误，在严重情况下 iterative self-refinement 会退化成 `self-deterioration`。[Latent Phase-Shift Rollback](https://arxiv.org/abs/2604.18567) 更激进：它不让模型用 prompt 自我纠错，而是在内部 KV-cache 层面回滚；论文摘要称，在它的实验设置里，这种方法比 prompted self-correction 高 24.2 个百分点。
...
很多人谈 multi-agent，会先谈并行、速度、覆盖面。这些当然重要。Anthropic 在 [一篇工程文章](https://www.anthropic.com/engineering/built-multi-agent-research-system) 里说，它们的 multi-agent research system 在内部 research eval 中比单 agent Claude Opus 4 高 90.2%。同一篇文章也说得很克制：multi-agent 很吃 token，任务必须真的可并行；coding 任务往往不像 research 那样天然适合拆很多独立方向。
