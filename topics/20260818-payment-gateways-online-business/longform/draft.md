...
| 数据层级 | 数量 | 用途 |
|---|---|---|
| 尝试查询的支付入口 | 14 个 | 确认哪些入口有来源数据 |
| 取得数据的入口 | 11 个，共 963 条记录 | 保留全体观察样本 |
| 专用结账入口 | 5 个，共 482 条记录 | 进入高置信统计 |
| 清理后的有效记录 | 459 条，约 454 个去重域名 | 计算支付活动信号与入口结构 |
| 完成行业分类的来源域名 | 329 个 | 计算来源域名数量占比 |
...
| 口径 | 怎样计算 | 它回答的问题 |
|---|---|---|
| 支付活动信号占比 | 五个入口内 459 条有效记录的估算信号合计 | 已采集来源中的活动主要集中在哪里 |
| 五个入口等权 | 每个入口先在内部归一化，再对五个入口取算术平均 | 减少 Stripe 体量影响后，各入口总体装着什么 |
| 已分类来源域名占比 | 329 个已分类域名跨入口去重，排除 125 个未分类域名 | 已分类来源域名的行业构成怎样 |

![同一批支付入口样本在三种统计口径下呈现不同结构](assets/chart-01-three-lenses.png)
...
![五个专用支付入口对应不同的行业结构](assets/chart-02-gateway-ecosystems.png)
...
高置信有效记录中有 117 条是新出现的来源。它们没有可比较的上月份额，因此不进入中位数计算。各行业被排除的比例并不相同。图中的圆点按实际参与中位数计算的有效样本数编码，旁边另列新出现来源的数量。中位数仍会受小基数、入口构成和缺失方式影响。

![各行业在支付入口内的来源份额变化中位数](assets/chart-03-relative-momentum.png)
...
行业标签只能回答一部分问题。第二轮核查了五类产品的价格、许可证、团队和销售流程。它们都出现在支付页附近，经营方法相差很大。

| 样本 | 卖什么 | 收费办法 | 经营结构 |
|---|---|---|---|
| RoughAnimator | 跨平台逐帧动画软件 | 每个平台 7.99 美元，一次性购买 | 一人开发，应用商店加桌面直销 |
| ADB AppControl | Android 设备管理工具 | 免费核心加单机永久解锁 | 独立开发，官网、YouTube 和 PayPro Global |
| Bombich | Mac 备份软件 | 49.99 到 125 美元许可证，试用和升级折扣 | 经营约 24 年的五人软件公司 |
| Poweramp | Android 本地音乐播放器 | 免费试用后终身解锁 | Google Play、官网 APK、直购和论坛 |
| AI Filmmakers | 两天 AI 影视直播课 | 每期 99 到 249 印度卢比 | 多讲师、多语言、多渠道滚动开课 |
...
本次统计采集于 2026 年 8 月 17 日，使用 Similarweb 2026 年 7 月全球、全部设备的 Incoming Referrals 数据，没有消耗 Similarweb credits。三张图的分母、计算方法、有效样本数和聚合结果见数据附录。产品价格、团队和页面信息核查截至 2026 年 8 月 18 日。

- [数据与计算附录](method-appendix.md)
- [研究总览](../../../research/20260817-payment-referral-opportunities/README.md)
- [第二轮公司与流量补证](../../../research/20260817-payment-referral-opportunities/ROUND2-RESEARCH.md)
