...
这不是 AI 一开始就能做到的。这是我花了一整天，把流程跑通、把坑踩完之后才有的结果。这篇文章想说的，就是那一天发生了什么——以及为什么流程建立之前和之后，效率完全不在一个量级。

<!-- more -->

## 任务背景

QCon 录了 8 场分享，加起来六个小时。录音转出来的逐字稿质量很差——这不是 ASR 的问题，而是现场录音本来就嘈杂，说话人会绕话、会引用专有名词、会用口语指代行业术语。

我想让 Claude Code 把这些东西整理成 9 篇可读笔记。

最终结果：9 篇全部完成，包括语篇规整、图片 OCR、同行审校、发布到内部知识库。但中间踩了不少坑。
...
AI 明白了。这句纠正花了三十秒，但它是后续所有事情能走通的前提。
...
但同一场，"他最早在这个 TDC 里面"，AI 推断成了 TED 演讲，实际是 ChatGPT 产品里。

再往后，第四场（百度）里，AI 把"COI化"推断成了"MCP化"，理由是"钉钉/悟空在做 MCP 化，时间线完全吻合"。

我看到这个推断就知道不对。COI 和 CLI 发音相近（C-O-I / C-L-I），演讲内容明显涉及命令行工具，MCP 差得太远了。
...
- 快猫星云那篇，末尾混入了网易那场的 Q&A（录音录到了相邻会场的声音，内容完全不属于这个话题）
- 淘宝闪购那篇，讲师自我介绍"大家好，我叫邓丽莎"——ASR 把"邓立山"识别错了，规整时没有发现
- PayPal 那篇，版本号被推断成了"Llama 3 Pro"——这个模型根本不存在，我听了录音确认是 Claude Opus 4.6、GPT-5.2 Codex、Gemini 3 Pro

做规整的 AI 看不出这些错误。它在规整时相信了自己的推断，没有质疑。独立的审校 agent 用新鲜的视角重新读，才发现了问题。

这就是"生成与验证分离"这个原则的价值：生成 agent 的目标是尽可能生成好的结果，验证 agent 的目标是尽可能发现问题，这两个目标天然有张力，放同一个 agent 里会相互妥协。

## 流程建好之后

把上面这些坑踩完、规则写进 SOP 之后，流程就稳了。

有了官方摘要，AI 处理每一场的节奏明显紧凑，用户介入次数更少。最后两篇，从逐字稿入库到规整完成——8 分钟。
...
还有另一类问题：文稿质量很难量化。"这句话还原得准不准"没有客观的数字答案，不像软件系统可以用可观测指标自动发现异常。独立审校 agent 发现问题之后，怎么改，改到什么程度，仍然需要人来判断。
